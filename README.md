...
Notion APIを活用した自作プログラム集です。
主に**インタースティシャル・ジャーナリングDB**の自動化と効率化を目的としています。
...
2. **プロパティIDによる指定**
   - Notion側で列名（例：「完了したこと」→「実績」）を変更してもプログラムが壊れないよう、名前ではなく固有ID（`title`, `I%5BJp` 等）を使用して操作する仕組みを導入しました。
